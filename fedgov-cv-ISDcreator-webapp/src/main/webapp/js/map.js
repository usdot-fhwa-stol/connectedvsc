import {  addRow, deleteRow, getCookie, makeDroppable, rebuildConnections, removeSpeedForm, resetSpeedDropdowns, saveConnections, saveSpeedForm, setLaneAttributes, toggle, toggleBars, toggleLanes, toggleLaneTypeAttributes, togglePoints, toggleWidthArray, unselectFeature, updateSharedWith, updateTypeAttributes } from "./utils.js";
import {newChildMap, newParentMap, openChildMap, openParentMap, selected, updateChildParent}  from "./parent-child.js"
import { revisionNum, saveMap, toggleControlsOn } from "./files.js";
...
function registerMapInteraction(){
  ...
  laneSelectInteraction.on('select', (evt)=>{ laneSelectInteractionCallback(evt, overlayLayersGroup, lanes, laneWidths, deleteMode, selected) });
  ...
  laneMarkersInteraction.on('select', (evt) => {
    selectedLayer = laneMarkers;
    selectedMarker = laneMarkersInteractionCallback(evt, overlayLayersGroup, lanes, laneConnections, deleteMode, selected, speedForm);
    if(selectedMarker){
  ...
  vectorAddInteraction.on('select', (evt) => {
    selectedLayer = vectors;
    selectedMarker = vectorSelectInteractionCallback(evt, overlayLayersGroup, lanes, deleteMode, selected, rgaEnabled, speedForm)
  });
  map.addInteraction(vectorAddInteraction);

  //Add feature event on vectors layer
  vectors.getSource().on("addfeature", evt => {
    selectedLayer = vectors;
    selectedMarker = vectorAddInteractionCallback(evt, selected, rgaEnabled, speedForm);
  })
  ...
  boxSelectInteraction.on('select', (evt) => {
    selectedLayer = box;
    selectedMarker = boxSelectInteractionCallback(evt, overlayLayersGroup, lanes, deleteMode, selected)
  });
...
function initSideBar() {  
  ...
  function clone(object, pixel) {
    ...
    let cloned_feature = new ol.Feature(new ol.geom.Point(coordinate));
    let IconInfo = {src: object.src, height: 50, width: 50};
    cloned_feature.setStyle(new ol.style.Style({
      image: new ol.style.Icon(IconInfo)
    }));
    let lonLat = ol.proj.toLonLat(coordinate);
    let lonLatObj = { lon: lonLat[0], lat: lonLat[1] };
    cloned_feature.set("LonLat", lonLatObj);
    cloned_feature.set("verifiedLat", cloned_feature.get("LonLat").lat);
    cloned_feature.set("verifiedLon", cloned_feature.get("LonLat").lon);

    let intersection_id = parseInt(object.id.match(/(\d+)$/)[0], 10);
    cloned_feature.set('marker',intersection_features[intersection_id]);
    vectors.getSource().addFeature(cloned_feature);
  }
}
...
function initMISC() {  
  ...
  */
  ...
  $(document).on('click', '[id^=rowDelete]', function() {
    let id = $(this).attr('id').replace(/^rowDelete/, '');
    deleteRow(id);
  });
  ...
  $(".btnDone").click(()=>{
    ...
    if ( $(".parsley-errors-list li:visible").length === 0 && dropdownCheck === true) {
      ...
      if (selectedLayer.get("title") == "Lane Marker Layer" && computingLane){
        ...
        buildComputedFeature(lanes.getSource().getFeatures().length, laneNum,
      ...
      let move = ol.proj.fromLonLat([parseFloat($('#long').val()), parseFloat($('#lat').val())]);
      if (selectedLayer.get("title") == "Lane Marker Layer"){   
        ...
        selectedMarker.set("LatLon", { lat: parseFloat($('#lat').val()), lon: parseFloat($('#long').val()) });
      ...
      if (selectedLayer.get("title") == "Vector Layer"){
        ...
        if (selectedMarker.get("marker").name == "Reference Point Marker") {
          ...
          selectedMarker.set('roadAuthorityIdType', $("#road_authority_id_type").val());
          ...
          selectedMarker.set('mappedGeometryId', $("#mapped_geometry_id").val());
  ...
  $(".btnClone").click(()=>{
    ...
    toggleControlsOn("placeComputed",lanes, vectors, laneMarkers, laneWidths, false);
